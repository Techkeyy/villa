...
for (const name of ["index.html", "styles.css", "app.mjs"]) await fs.copyFile(path.join(source, name), path.join(output, name));
await fs.copyFile(path.join(root, "src", "dashboard", "presenter.mjs"), path.join(output, "presenter.mjs"));

const html = await fs.readFile(path.join(output, "index.html"), "utf8");
const required = ["/styles.css", "/app.mjs", "VILLA", "REPLAY", "PNL_UNAVAILABLE"];
